...
ghcr.io/siginsight/siginsight-otel-collector:v1.2.0
ghcr.io/siginsight/clickhouse-init-histogram-quantile:25.5.6-latest
```

The complete deployment currently supports Linux AMD64 hosts only because the `v1.2.0` collector image does not provide an ARM64 variant.

You can override their tags with environment variables:

```sh
export VERSION=v1.6.2
export OTELCOL_TAG=v1.2.0
