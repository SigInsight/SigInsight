...
import {
	...
	EmailChannel,
	WebhookChannel,
} from '../CreateAlertChannels/config';
...
function EditAlertChannels({
	...
	const id =
		window.location.pathname.match(/\/settings\/channels\/edit\/([^/]+)/)?.[1] ||
		'';
	...
	const save = async (): Promise<void> => {
		setSaving(true);
		try {
			if (initialValue.type === ChannelType.Email) {
				await editEmail({ ...emailRequest(), id });
			} else {
				await editWebhook({ ...webhookRequest(), id });
			}
	...
	return (
		<FormAlertChannels
			...
			onTypeChangeHandler={(): void => undefined}
			onSaveHandler={save}
			onTestHandler={test}
			savingState={saving}
