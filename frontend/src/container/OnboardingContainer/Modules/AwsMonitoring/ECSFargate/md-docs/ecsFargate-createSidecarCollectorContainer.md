...
This step involves integrating the SigInsight collector into your ECS task definitions as a sidecar container. The sidecar collector container will run alongside your application container(s) within the same ECS task and will collect ECS container metrics and send them to SigInsight Cloud. Follow these steps to create the Sidecar collector container:
