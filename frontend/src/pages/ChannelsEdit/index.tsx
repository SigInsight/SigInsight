...
import { ChannelConfig, ChannelType } from 'container/CreateAlertChannels/config';
...
function ChannelsEdit(): JSX.Element {
	...
	const channelId = window.location.pathname.match(/\/settings\/channels\/edit\/([^/]+)/)?.[1];
	const { isFetching, isError, data, error } = useQuery<HttpSuccessResponse<Channels>, APIError>(
	...
	if (isError) return <Typography>{error?.getErrorMessage() || t('something_went_wrong')}</Typography>;
	if (isFetching || !data?.data) return <Spinner tip="Loading Channels..." />;
