...
import { normalizeQueryRangeResponse } from './normalizeQueryRangeResponse';

describe('normalizeQueryRangeResponse', () => {
	...
	it('converts time_series response into legacy series structure', () => {
		...
		const result = normalizeQueryRangeResponse(input, legendMap, false);
	...
	it('converts scalar with formatForWeb=true to UI-friendly table', () => {
		...
		> = makeBaseSuccess({ data: v5Data }, params);
		const legendMap = { A: '{{service.name}}' };
		const result = normalizeQueryRangeResponse(input, legendMap, true);
