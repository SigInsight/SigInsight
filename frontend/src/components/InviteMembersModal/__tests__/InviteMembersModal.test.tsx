...
describe('InviteMembersModal', () => {
	...
	it('adds a row when "Add another" is clicked and removes a row via trash button', async () => {
		...
		expect(screen.getAllByPlaceholderText('john@example.com')).toHaveLength(4);

		const removeButtons = screen.getAllByRole('button', { name: /remove row/i });
		await user.click(removeButtons[0]);
		expect(screen.getAllByPlaceholderText('john@example.com')).toHaveLength(3);
	});
	...
	describe('validation callout messages', () => {
		it('shows combined message when email is invalid and role is missing', async () => {
			...
			await user.type(
				screen.getAllByPlaceholderText('john@example.com')[0],
				'not-an-email',
			);
			await user.click(
				screen.getByRole('button', { name: /invite team members/i }),
	...
	it('uses sendInvite (single) when only one row is filled', async () => {
		...
		const emailInputs = screen.getAllByPlaceholderText('john@example.com');
		await user.type(emailInputs[0], 'single@example.com');
		...
		await waitFor(() => {
			expect(mockSendInvite).toHaveBeenCalledWith(
				expect.objectContaining({ email: 'single@example.com', role: 'VIEWER' }),
			);
		...
		await waitFor(() => {
			expect(mockInviteUsers).toHaveBeenCalledWith({
				invites: expect.arrayContaining([
					expect.objectContaining({ email: 'alice@example.com', role: 'VIEWER' }),
					expect.objectContaining({ email: 'bob@example.com', role: 'EDITOR' }),
				]),
			});
