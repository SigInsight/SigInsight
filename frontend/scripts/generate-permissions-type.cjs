...
const SIGINSIGHT_INTEGRATION_IMAGE = 'siginsight:integration';
...
function getBackendUrlFromDocker() {
	try {
		const output = execSync(
			`docker ps --filter "ancestor=${SIGINSIGHT_INTEGRATION_IMAGE}" --format "{{.Ports}}"`,
...
async function main() {
	try {
		...
		if (!backendUrl) {
			console.error('\n' + '='.repeat(80));
			console.error('ERROR: No running SigInsight backend found!');
			console.error('='.repeat(80));
			console.error(
				'\nThe permissions type generator requires a running SigInsight backend.',
			);
