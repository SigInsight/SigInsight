...
func (provider *provider) collectOrg(ctx context.Context, orgID valuer.UUID) map[string]any {
	...
	var traces uint64
	if err := provider.telemetryStore.ClickhouseDB().QueryRow(ctx, "SELECT COUNT(*) FROM siginsight_traces.spans").Scan(&traces); err == nil {
		stats["telemetry.traces.count"] = traces
	}

	var logs uint64
	if err := provider.telemetryStore.ClickhouseDB().QueryRow(ctx, "SELECT COUNT(*) FROM siginsight_logs.logs").Scan(&logs); err == nil {
		stats["telemetry.logs.count"] = logs
	}

	var metrics uint64
	if err := provider.telemetryStore.ClickhouseDB().QueryRow(ctx, "SELECT COUNT(*) FROM siginsight_metrics.metric_points").Scan(&metrics); err == nil {
		stats["telemetry.metrics.count"] = metrics
	}

	var tracesLastSeenAt time.Time
	if err := provider.telemetryStore.ClickhouseDB().QueryRow(ctx, "SELECT max(timestamp) FROM siginsight_traces.spans").Scan(&tracesLastSeenAt); err == nil {
	...
	if err := provider.telemetryStore.ClickhouseDB().QueryRow(ctx, "SELECT fromUnixTimestamp64Nano(max(timestamp)) FROM siginsight_logs.logs").Scan(&logsLastSeenAt); err == nil {
	...
	if err := provider.telemetryStore.ClickhouseDB().QueryRow(ctx, "SELECT toDateTime(max(unix_milli) / 1000) FROM siginsight_metrics.metric_points").Scan(&metricsLastSeenAt); err == nil {
