	...
	tests := []struct {
		...
		},
		{
		    ...
		    toStartOfInterval(toDateTime(intDiv(unix_milli, 1000)), toIntervalSecond(60)) as ts,
		        ...
		        rand() as value
		    FROM siginsight_metrics.time_series_v4
	...
	tests := []struct {
		...
		WITH __spatial_aggregation_cte AS    (
		        ...
		        toStartOfInterval(toDateTime(intDiv(unix_milli, 1000)), toIntervalSecond(60)) AS ts,
		            ...
		            FROM siginsight_metrics.samples_v4 AS points
		        INNER JOIN   (
		            SELECT
		            fingerprint,
		...
		SELECT
		    ...
		    SELECT
		        ...
		        FROM (
		            ...
		            FROM siginsight_metrics.samples_v4
		            INNER JOIN (
		                SELECT DISTINCT
		                    JSONExtractString(labels, 'le') AS le,
		                    fingerprint
		...
		WITH  toUInt64(1650991980000) AS start_ms,
		  ...
		  FROM siginsight_metrics.samples_v4 AS s
		  JOIN siginsight_metrics.time_series_v4_1day AS tsv
