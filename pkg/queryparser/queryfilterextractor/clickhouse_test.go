	...
	tests := []struct {
		...
		},
		{
		    ...
		    toStartOfInterval(toDateTime(intDiv(unix_milli, 1000)), toIntervalSecond(60)) as ts,
		        ...
		        rand() as value
		    FROM siginsight_metrics.metric_series
	...
	tests := []struct {
		...
		WITH __spatial_aggregation_cte AS    (
		        ...
		        toStartOfInterval(toDateTime(intDiv(unix_milli, 1000)), toIntervalSecond(60)) AS ts,
		            ...
		            FROM siginsight_metrics.metric_points AS points
		        INNER JOIN   (
		            SELECT
		            fingerprint,
		...
		SELECT
		    ...
		    SELECT
		        ...
		        FROM (
		            ...
		            FROM siginsight_metrics.metric_points
		            INNER JOIN (
		                SELECT DISTINCT
		                    JSONExtractString(labels, 'le') AS le,
		                    fingerprint
		                FROM siginsight_metrics.metric_series_1d
		                WHERE
		...
		WITH  toUInt64(1650991980000) AS start_ms,
		  ...
		  FROM siginsight_metrics.metric_points AS s
		  JOIN siginsight_metrics.metric_series_1d AS tsv
