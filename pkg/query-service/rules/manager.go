...
// TestNotifyFunc is the synchronous notification boundary used only by the
// explicit test-rule API. Scheduled evaluation remains best-effort, whereas a
// caller who requested a test notification must receive delivery failures.
type TestNotifyFunc func(ctx context.Context, orgID string, expr string, alerts ...*ruletypes.Alert) error
...
func (m *Manager) prepareTestNotifyFunc() TestNotifyFunc {
	return func(ctx context.Context, orgID string, expr string, alerts ...*ruletypes.Alert) error {
		if len(alerts) == 0 {
			return nil
		}
