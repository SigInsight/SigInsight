...
func ruleSeriesFromTimeSeries(series *qbtypes.TimeSeries) *timeseriestypes.Series {
	result := &timeseriestypes.Series{
		Labels:      make(map[string]string),
		LabelsArray: make([]map[string]string, 0, 1),
	}
	if series == nil {
		return result
	}

	result.Points = make([]timeseriestypes.Point, 0, len(series.Values))
	for _, label := range series.Labels {
		if label == nil {
			continue
		}
		result.Labels[label.Key.Name] = fmt.Sprint(label.Value)
	}
	if len(result.Labels) > 0 {
		result.LabelsArray = append(result.LabelsArray, result.Labels)
	}
...
func (r *ThresholdRule) buildAndRunQuery(ctx context.Context, orgID valuer.UUID, ts time.Time) (ruletypes.Vector, error) {
	...
	selectedQuery := r.GetSelectedQuery()
	queryFound := false
	seriesToProcess := make([]*timeseriestypes.Series, 0)
	...
	if v5Result != nil {
		for _, item := range v5Result.Data.Results {
			tsData, ok := item.(*qbtypes.TimeSeriesData)
			if !ok {
				r.logger.WarnContext(ctx, "expected qbtypes.TimeSeriesData", "item_type", fmt.Sprintf("%T", item))
				continue
			}
			if tsData == nil || tsData.QueryName != selectedQuery {
				continue
			}

			queryFound = true
			for _, aggregation := range tsData.Aggregations {
				if aggregation == nil {
					continue
				}
				for _, series := range aggregation.Series {
					seriesToProcess = append(seriesToProcess, ruleSeriesFromTimeSeries(series))
				}
			}
		}
	}
