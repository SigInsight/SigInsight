...
func Test_buildTracesQuery(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: "SELECT  attributes_string['http.method'] as `http.method`, toFloat64(count()) as value from signoz_traces.signoz_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') " +
		...
		},
		{
			...
			want: "SELECT  resources_number['host'] as `host`, toFloat64(count()) as value from signoz_traces.signoz_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') " +
				"AND (ts_bucket_start >= 1680064560 AND ts_bucket_start <= 1680066458) AND attributes_number['bytes'] > 100 AND " +
				"(resource_fingerprint GLOBAL IN (SELECT fingerprint FROM signoz_traces.traces_v3_resource WHERE (seen_at_ts_bucket_start >= 1680064560) AND " +
		...
		},
		{
			...
			want: "SELECT timestamp as timestamp_datetime, span_id as spanID, trace_id as traceID, name as `name` from signoz_traces.signoz_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') " +
		...
		},
		{
			...
			want: "SELECT timestamp as timestamp_datetime, span_id as spanID, trace_id as traceID, name as `name` from signoz_traces.signoz_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') " +
				"AND (ts_bucket_start >= 1680064560 AND ts_bucket_start <= 1680066458)  AND ((name, `resource_string_service$$name`) GLOBAL IN ( SELECT DISTINCT name, serviceName from signoz_traces.top_level_operations )) AND parent_span_id != ''  order by timestamp ASC",
		},
		{
		...
		},
		{
			...
			want: "SELECT subQuery.serviceName as `subQuery.serviceName`, subQuery.name as `subQuery.name`, count() AS span_count, subQuery.durationNano as `subQuery.durationNano`, subQuery.traceID FROM " +
				"(SELECT traceID AS dist_traceID, timestamp, ts_bucket_start FROM signoz_traces.signoz_index_v3 WHERE (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') AND (ts_bucket_start >= 1680064560 AND ts_bucket_start <= 1680066458) AND attributes_string['method'] = 'GET' " +
				"AND (resource_fingerprint GLOBAL IN (SELECT fingerprint FROM signoz_traces.traces_v3_resource WHERE (seen_at_ts_bucket_start >= 1680064560) AND (seen_at_ts_bucket_start <= 1680066458) AND simpleJSONExtractString(labels, 'service.name') = 'myService' AND labels like '%service.name\":\"myService%'))) as dist_table " +
				"INNER JOIN " +
				...
				"subQuery.durationNano desc LIMIT 1 BY subQuery.traceID  LIMIT 100 settings max_memory_usage=10000000000",
		},
		{
			...
			want: "SELECT serviceName, name, subQuery.span_count as span_count, durationNano, trace_id as traceID from signoz_traces.signoz_index_v3 GLOBAL INNER JOIN " +
				...
				"AND attributes_string['method'] = 'GET' AND (resource_fingerprint GLOBAL IN (SELECT fingerprint FROM signoz_traces.traces_v3_resource WHERE " +
				...
				"GROUP BY trace_id ORDER BY span_count DESC LIMIT 1 BY trace_id LIMIT 100) AS inner_subquery ) AS subQuery ON signoz_traces.signoz_index_v3.trace_id = subQuery.trace_id " +
				...
				"ORDER BY subQuery.span_count DESC LIMIT 100 settings max_memory_usage=10000000000",
		},
		{
			...
			want: "SELECT  toFloat64(count(distinct(name))) as value from signoz_traces.signoz_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') AND " +
		...
		},
		{
			...
			want: "SELECT  max(toUnixTimestamp64Nano(timestamp)) as value from signoz_traces.signoz_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') AND " +
		...
		},
		{
			...
			want: "SELECT toStartOfInterval(timestamp, INTERVAL 60 SECOND) AS ts, count()/60.000000 as value from signoz_traces.signoz_index_v3 where (timestamp >= '1745315470000000000' AND " +
		...
		},
		{
			...
			want: "SELECT  count()/3600.000000 as value from signoz_traces.signoz_index_v3 where (timestamp >= '1745315470000000000' AND " +
...
func TestPrepareTracesQuery(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: "SELECT `function` from (SELECT attributes_string['function'] as `function`, toFloat64(count(distinct(name))) as value from signoz_traces.signoz_index_v3 " +
		...
		},
		{
			...
			want: "SELECT  attributes_string['function'] as `function`, toFloat64(count(distinct(name))) as value from signoz_traces.signoz_index_v3 where " +
		...
		},
		{
			...
			want: "SELECT `function`,`service.name` from (SELECT `attribute_string_function` as `function`, `resource_string_service$$name` as `service.name`, toFloat64(count(distinct(name))) as value " +
				"from signoz_traces.signoz_index_v3 where (timestamp >= '1680066360726210000' AND timestamp <= '1680066458000000000') AND (ts_bucket_start >= 1680064560 AND ts_bucket_start <= 1680066458) " +
				"AND attributes_number['line'] = 100 AND (resource_fingerprint GLOBAL IN (SELECT fingerprint FROM signoz_traces.traces_v3_resource WHERE " +
		...
		},
		{
			...
			want: "SELECT  `attribute_string_function` as `function`, resource_string_service$$name as `serviceName`, toFloat64(count(distinct(name))) as value from signoz_traces.signoz_index_v3 " +
				...
				"AND (resource_fingerprint GLOBAL IN (SELECT fingerprint FROM signoz_traces.traces_v3_resource WHERE (seen_at_ts_bucket_start >= 1680064560) AND (seen_at_ts_bucket_start <= 1680066458) " +
