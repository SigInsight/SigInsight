...
func AddMetricValueFilter(mq *querytypes.BuilderQuery) *querytypes.MetricValueFilter {

	var metricValueFilter *querytypes.MetricValueFilter = nil
	...
	if mq != nil && mq.Filters != nil && mq.Filters.Items != nil {
		for _, item := range mq.Filters.Items {
			if item.Key.Key == "__value" {
				...
				case float64:
					metricValueFilter = &querytypes.MetricValueFilter{
						Value: v,
					}
				case float32:
					metricValueFilter = &querytypes.MetricValueFilter{
