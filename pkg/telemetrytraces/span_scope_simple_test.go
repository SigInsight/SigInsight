...
func TestSpanScopeFilterExpression(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedCondition: "((name, service_name) GLOBAL IN (SELECT DISTINCT name, serviceName from siginsight_traces.operations)) AND parent_span_id != ''",
			startNs:           0,
		},
		{
			name:              "simple isentrypoint filter (bounded)",
			expression:        "isentrypoint = true",
			expectedCondition: "((name, service_name) GLOBAL IN (SELECT DISTINCT name, serviceName from siginsight_traces.operations WHERE time >= toDateTime(1761437108))) AND parent_span_id != ''",
			startNs:           1761437108000000000,
		},
		{
