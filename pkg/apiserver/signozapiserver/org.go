...
func (provider *provider) addOrgRoutes(router *mux.Router) error {
	if err := router.Handle("/api/v5/orgs/me", handler.New(provider.authZ.ViewAccess(provider.orgHandler.Get), handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v5/orgs/me", handler.New(provider.authZ.AdminAccess(provider.orgHandler.Update), handler.OpenAPIDef{
